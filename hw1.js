...
const Product = class Product {
    ...
    constructor(val){
        ...
        this.price = Number(this.val[1]);
        this.availability = this.val[2];
    }
     ...
     */
     static inStock = (products) => {
        const inStockProduct = new Array(0);

        products.forEach((element) => {
            if (Object.values(element).includes('In Stock')) {
                inStockProduct.push(element);
            }
        });
    ...
    static halfOff = (products) => {
        ...
        let price;
        let availability;
        let reducedCostArray = new Array(0);

        products.forEach(element => {
            name = element.name;
            price = element.price;
            availability = element.availability;
            reducedCostArray.push({test: name, price: price * 0.5, test2: availability});
        })
        return reducedCostArray;
    }
    ...
    pArray.forEach(element => {
       ...
       if (element.availability === "In Stock") {
        ...
        console.log(`Product: ${product}, Cost: ${nf.format(cost)}, Availability: ${availability}`);
    })
    }
 };
